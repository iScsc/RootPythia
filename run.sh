...
run__dev () {
	...
	docker run --rm --interactive --tty --env-file .env.dev --volume $(pwd -P ./src):/opt/root-pythia/src root-pythia-dev:latest
}

run__watch () {
	# watch mode
	docker build --file Dockerfile.watch -t root-pythia-watch:latest .
	docker run --rm --interactive --tty --env-file .env.dev --volume $(pwd -P ./src):/opt/root-pythia/src root-pythia-watch:latest
}
